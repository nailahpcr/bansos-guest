            ...
            <div class="row">
                <div class="col-12">
                    <div class="section-title">
                        ...
                        <p class="wow fadeInUp" data-wow-delay=".6s">Masukan hasil survei dan penilaian verifikasi data
                            pendaftar di bawah ini.</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card shadow-sm wow fadeInUp" data-wow-delay=".8s">
                        <div class="card-body p-4">
                            ...
                            <form action="{{ route('verifikasi.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    <label for="pendaftar_id" class="form-label"><strong>Nama Pendaftar
                                            (Warga):</strong></label>
                                    <select name="pendaftar_id" id="pendaftar_id"
                                        class="form-control @error('pendaftar_id') is-invalid @enderror">
                                        <option value="">-- Pilih Pendaftar --</option>
                                        @foreach ($pendaftar as $item)
                                            <option value="{{ $item->pendaftar_id }}"
                                                {{ old('pendaftar_id') == $item->pendaftar_id ? 'selected' : '' }}>
                                                {{ $item->warga->nama ?? 'Warga ID: ' . $item->warga_id }} -
                                                ({{ $item->program->nama_program ?? 'Program Lain' }})
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('pendaftar_id')
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="petugas" class="form-label"><strong>Nama Petugas
                                                Verifikator:</strong></label>
                                        <input type="text" name="petugas" id="petugas"
                                            class="form-control @error('petugas') is-invalid @enderror"
                                            placeholder="Nama petugas survei" value="{{ old('petugas') }}">
                                        @error('petugas')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                {{-- Field Skor --}}
                                <div class="mb-3">
                                    <label for="skor" class="form-label"><strong>Skor Penilaian
                                            (0-100):</strong></label>
                                    <input type="number" name="skor" id="skor"
                                        class="form-control @error('skor') is-invalid @enderror" placeholder="Contoh: 85"
                                        value="{{ old('skor') }}">
                                    @error('skor')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="catatan" id="catatan" class="form-control @error('catatan') is-invalid @enderror" style="height:100px"
                                        placeholder="Tuliskan temuan atau catatan penting di lapangan...">{{ old('catatan') }}</textarea>
                                    @error('catatan')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <small class="text-muted">Bisa memilih lebih dari 1 gambar sekaligus</small>
                                    @error('file')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
