...
@section('content')
    <style>
        ...
        body {
            background: linear-gradient(180deg, #ff5776 0%, #ffffff 100%);
            min-height: 100vh;
    ...
    <section id="features" class="features section">
        <div class="container">
            ...
            <div class="section-title text-center">
                ...
                <h2 class="wow fadeInUp text-black" data-wow-delay=".4s">Hasil Verifikasi Lapangan</h2>
                <p class="wow fadeInUp text-white" data-wow-delay=".6s">Kelola dan tinjau hasil survei untuk bantuan tepat
                    sasaran.</p>
            </div>
                        ...
                        <form action="{{ route('verifikasi.index') }}" method="GET" class="flex-grow-1">
                            <div class="search-combined-group shadow-sm">
                                <select name="status" class="form-select border-end" onchange="this.form.submit()">
                                    ...
                                    <option value="layak" {{ request('status') == 'layak' ? 'selected' : '' }}>Layak (≥ 70)
                                    </option>
                                    <option value="kurang" {{ request('status') == 'kurang' ? 'selected' : '' }}>Kurang (<
                                            70)</option>
                                </select>
                                ...
                                <input type="text" name="q" class="form-control" placeholder="Cari Nama Warga..."
                                    value="{{ request('q') }}">
            ...
            <div class="row g-4">
                @forelse ($verifikasi as $item)
                    @php $uniqueId = $item->verifikasi_id ?? $item->id; @endphp
                        ...
                        <div class="card h-100 riwayat-card">
                            ...
                            <div class="riwayat-image-wrapper">
                                <div class="badge-score-float">
                                    <span
                                        class="badge {{ $item->skor >= 70 ? 'bg-success' : 'bg-warning' }} rounded-pill px-3 py-2 shadow-sm">
                                        Skor: {{ $item->skor }}
                                    </span>
                                </div>

                                {{-- Logika Tampilan Berkas (Gambar vs PDF) --}}
                                @php
                                    $firstFile = $item->files->first();
                                    $extension = $firstFile
                                        ? strtolower(pathinfo($firstFile->path, PATHINFO_EXTENSION))
                                        : null;
                                @endphp
                                ...
                                @if ($firstFile)
                                    @if (in_array($extension, ['jpg', 'jpeg', 'png', 'webp']))
                                        {{-- Jika Gambar --}}
                                        <img src="{{ asset('storage/' . $firstFile->path) }}" alt="Bukti Lapangan">
                                    @elseif ($extension == 'pdf')
                                        {{-- Jika PDF: Tampilkan Ikon PDF Besar --}}
                                        <div class="text-center">
                                            <i class="fas fa-file-pdf fa-5x text-danger opacity-75"></i>
                                            <p class="mb-0 mt-2 small text-muted fw-bold">DOKUMEN PDF</p>
                                        </div>
                                    @else
                                        {{-- Jika File Lain --}}
                                        <div class="text-center">
                                            <i class="fas fa-file-alt fa-5x text-muted opacity-50"></i>
                                            <p class="mb-0 mt-2 small text-muted">Berkas Lainnya</p>
                                        </div>
                                    @endif
                                @else
                                    {{-- Jika Tidak Ada File --}}
                                    <div class="text-center opacity-25">
                                        ...
                                        <p class="mb-0 mt-2">Tidak ada berkas</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="card-body p-4 d-flex flex-column">
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="badge bg-soft-primary text-primary"
                                        style="font-size: 0.7rem; font-weight: 800; background-color: rgba(13, 110, 253, 0.1); padding: 5px 10px;">VERIFIKASI</span>
                                    <small class="text-muted"><i
                                            class="far fa-calendar-alt me-1"></i>{{ \Carbon\Carbon::parse($item->tanggal)->format('d/m/Y') }}</small>
                                </div>
                                ...
                                {{-- LIST FILE --}}
                                <div class="mb-3">
                                    <small class="text-muted d-block mb-2"><i class="fas fa-paperclip me-1"></i> Lampiran
                                        ({{ $item->files->count() }})
                                        :</small>
                                    @forelse ($item->files->take(2) as $file)
                                        <div class="d-flex align-items-center mb-1 bg-light p-1 rounded border"
                                            ...
                                            <i
                                                class="fas {{ str_ends_with($file->path, '.pdf') ? 'fa-file-pdf text-danger' : 'fa-image text-info' }} me-2"></i>
                                ...
                                <div class="mb-3">
                                    ...
                                    <span class="fw-bold small"><i class="fas fa-user-check me-1 text-secondary"></i>
                                        {{ $item->petugas }}</span>
                                </div>
                                ...
                                <div class="mt-auto pt-3 border-top">
                                    <div class="btn-group w-100 gap-2">
                                        ...
                                        <button type="button" class="btn btn-sm btn-light text-danger rounded-3"
                                            ...
                                            data-bs-toggle="modal" data-bs-target="#modalHapus{{ $uniqueId }}">
                    ...
                    {{-- MODAL DELETE: Letakkan di luar Card tapi di dalam @forelse --}}
                    <div class="modal fade" id="modalHapus{{ $uniqueId }}" tabindex="-1" aria-hidden="true"
                        style="z-index: 9999;">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content border-0 shadow">
                                ...
                                <form action="{{ route('verifikasi.destroy', $uniqueId) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <div class="modal-body text-center pt-0">
                                        ...
                                        <p class="text-muted">Apakah Anda yakin ingin menghapus data verifikasi milik:</p>
                                        <p class="fw-bold fs-5 text-dark">{{ $item->pendaftar->warga?->nama ?? 'N/A' }}
                                        </p>
                                    </div>
                                    <div class="modal-footer border-0 justify-content-center pb-4">
                                        <button type="button" class="btn btn-light rounded-pill px-4"
                                            data-bs-dismiss="modal">Batal</button>
                                        <button type="submit" class="btn btn-danger rounded-pill px-4">Ya, Hapus
                                            Data</button>
                                    </div>
                                </form>
                            </div>
