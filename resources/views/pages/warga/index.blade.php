...
<style>
    ...
    .features.section {
        background: transparent; /* Memastikan background section tidak menutupi gradasi body */
    }
    ...
    .btn-add-warga:hover { background: #27ae60; color: #fff; transform: translateY(-2px); }

    /* 4. Statistics Cards */
    .stat-card { transition: 0.3s; border-radius: 15px; overflow: hidden; }
    ...
    /* 5. Citizen Cards */
    .citizen-card { transition: 0.3s; border-radius: 15px; }
    .citizen-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0,0,0,0.1) !important; }
    ...
    @media (max-width: 991px) {
        ...
        .btn-add-warga { justify-content: center; }
    }
</style>
...
<section id="features" class="features section">
    <div class="container">
        <div class="row">
            <div class="col-12 text-center mb-5">
                <div class="section-title">
                    ...
                    <p class="wow fadeInUp  text-white" data-wow-delay=".6s">Kelola informasi kependudukan dengan cepat dan akurat.</p>
                </div>
            </div>
        </div>
                ...
                <div class="action-bar-container shadow-sm">
                    <form action="{{ route('warga.index') }}" method="GET" class="flex-grow-1">
                        <div class="search-combined-group">
                            <select name="gender" class="form-select shadow-none">
                                <option value="" {{ request('gender') == '' ? 'selected' : '' }}>Semua Gender</option>
                                <option value="Laki-laki" {{ request('gender') == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                                <option value="Perempuan" {{ request('gender') == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                            </select>
                            <input type="text" name="search" class="form-control shadow-none" placeholder="Cari NIK atau Nama..." value="{{ request('search') }}">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        @if (request('search') || request('gender'))
                            <a href="{{ route('warga.index') }}" class="btn btn-light border d-flex align-items-center justify-content-center" style="width: 48px; border-radius: 12px;">
        ...
        <div class="row g-4 mb-5">
            @php 
                $cards = [
                    ['label' => 'Total Warga', 'value' => $totalWarga, 'icon' => 'fa-users', 'color' => 'text-primary', 'bg' => 'rgba(13, 110, 253, 0.1)'],
                    ['label' => 'Laki-laki', 'value' => $totalLakiLaki, 'icon' => 'fa-male', 'color' => 'text-info', 'bg' => 'rgba(13, 202, 240, 0.1)'],
                    ['label' => 'Perempuan', 'value' => $totalPerempuan, 'icon' => 'fa-female', 'color' => 'text-danger', 'bg' => 'rgba(220, 53, 69, 0.1)']
                ];
            @endphp
                ...
                <div class="card stat-card border-0 shadow-sm">
                    <div class="card-body p-4 position-relative">
                        <div class="d-flex align-items-center position-relative" style="z-index: 2;">
                            <div class="icon-box rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 60px; height: 60px; background: {{ $card['bg'] }};">
                                <i class="fas {{ $card['icon'] }} fa-2x {{ $card['color'] }}"></i>
                            </div>
                            <div>
                                <div class="text-uppercase mb-1 fw-bold {{ $card['color'] }}" style="font-size: 0.7rem; letter-spacing: 1px;">{{ $card['label'] }}</div>
                                <h2 class="h3 mb-0 fw-bold text-dark">{{ number_format($card['value']) }}</h2>
                            </div>
                        </div>
                        <div class="bg-decoration" style="color: {{ str_contains($card['color'], 'primary') ? '#0d6efd' : (str_contains($card['color'], 'info') ? '#0dcaf0' : '#dc3545') }}"></div>
                    </div>
                </div>
            </div>
            @endforeach
                ...
                <div class="col-md-6 col-lg-4 wow fadeInUp">
                    <div class="card citizen-card border-0 shadow-sm h-100">
                        <div class="card-body p-4">
                            ...
                            <h6 class="card-subtitle mb-3 text-muted small"><i class="fas fa-id-card me-1"></i> {{ $warga->no_ktp }}</h6>
                            <hr class="my-3 opacity-25">
                            
                            <div class="mb-4">
                                <p class="mb-1 small text-muted"><i class="fas fa-venus-mars me-2"></i>{{ $warga->jenis_kelamin }}</p>
                                <p class="mb-1 small text-muted"><i class="fas fa-pray me-2"></i>{{ $warga->agama }}</p>
                                <p class="mb-0 small text-muted"><i class="fas fa-phone me-2"></i>{{ $warga->telp ?? '-' }}</p>
                            </div>

                            <div class="btn-group w-100 gap-2">
                                <a href="{{ route('warga.show', $warga->warga_id) }}" class="btn btn-light-info flex-fill rounded-3 py-2"><i class="fas fa-eye"></i></a>
                                <a href="{{ route('warga.edit', $warga->warga_id) }}" class="btn btn-light-warning flex-fill rounded-3 py-2"><i class="fas fa-edit"></i></a>
                                <button type="button" class="btn btn-light-danger flex-fill rounded-3 py-2" data-bs-toggle="modal" data-bs-target="#del{{ $warga->warga_id }}"><i class="fas fa-trash"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-sm">
                        <div class="modal-content border-0 shadow">
                            <div class="modal-body text-center p-4">
                                ...
                                <p class="small text-muted">Yakin ingin menghapus <strong>{{ $warga->nama }}</strong>?</p>
